# Tutorial

Welcome to the Sviter tutorial. You'll learn by doing — right here on this page.

---

## Step 1: Edit a Page

Click on any page in the left panel. Click the **Edit**

**Try it now:**

1. Click the **Edit**
2. Edit the text below — add your name or a word

> Hello, I am learning Sviter! My name is \__\_.

Your changes are saved automatically. If another user is viewing this page, they see your changes in real-time — like Google Docs. The cloud icon at the top shows the sync status.

---

## Step 2: Chat Basics

Look at the **right panel**

**Try it now:**

1. Click the chat input at the bottom of the right panel
2. Type: `What pages exist in this wiki?`
3. Press Enter

The assistant will search the wiki and list all pages. It can read any page and answer questions about the content.

**Try another:**

- `Summarize the Home page`
- `What is Sviter?`

---

## Step 3: Context Selection

You can send specific text to the chat as context.

**Try it now:**

1. Select the sentence below with your mouse
2. A button appears: **"Add to Chat"**
3. Click it
4. Now type: `Can you rephrase this?`

> Sviter helps teams work together on documentation without conflicts.

The assistant sees exactly what you selected and can work with it.

---

## Step 4: Spawning a Thread

When you want the AI to make actual changes, it spawns a **worker thread**

The thread works on its own branch — your main content stays safe. All AI work will be reviewed by you before it merges.

**Try it now:**

Select the text below and ask the assistant: `Make this more concise and clear`

> In order to be able to successfully utilize the functionality of creating new pages, the user must first navigate to the left panel area and then proceed to right-click on the desired location where the new page should be created.

Watch the right panel. You'll see:

1. A new thread appears in the **Threads**
2. Status: **WORKING**
3. When done: **REVIEW**

---

## Step 5: Review Changes

Click on the thread (or switch to the **Threads**

The center panel shows a **diff view**

- Red = removed
- Green = added

You see exactly what the agent changed. Multiple users can review and discuss in the thread chat before deciding.

---

## Step 6: Accept or Reject

At the bottom of the review, you have two buttons:

- **Accept** — Merge the changes to main
- **Reject** — Discard the branch, keep original

**Try it:**

- If the fix looks good, click **Accept**
- The typo fix merges into this page
- Reload to see the result

---

## You Did It!

You've learned the core Sviter workflow:

1. **Edit pages** — real-time sync with other users
2. **Chat** with the assistant to find information
3. **Select text** to give context
4. **Spawn threads** for AI changes
5. **Review diffs** — all AI work is reviewed by you
6. **Accept or Reject** with one click

Git handles everything under the hood. You never have to touch it.

---

## Next Steps

- [Concepts](Concepts) — Deeper understanding of threads and branches
- [User Guide](User-Guide) — Full feature reference